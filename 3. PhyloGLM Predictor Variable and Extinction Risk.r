...
Master_Dataset <- Master_Dataset %>% mutate_all(na_if,"")
...
#Generate Figure 1
Best_Model_Coef<-ggplot(coef_df2, aes(x=X, y=coef)) + geom_violin(fill="#31AE3B")+
  scale_y_continuous(breaks=seq(-.5,1.5,.5),limits = c(-.75, 1.5))  +
  stat_summary(
    geom = "point",
    fun = "median",
    col = "black",
...
#*Note we ended up dropping the GLM analysis from the paper, keeping in as legacy code

#Running GLMs on same dataset to compare how phyloGLM and GLMs perform.

#Making sure to feed in the same set of species (ie same dataset) - phylogenetic
#matching drops out a few species

Phylo_list<-lapply(treedat,'[[',1)

Phylo_species<-lapply(Phylo_list,'[[',4)

Phylo_species2<-Phylo_species[[1]]

Phylo_species3<-data.frame(Phylo_species2)

Phylo_species4 <- Phylo_species3 %>% rename(scientific_name = Phylo_species2)


GLM_df <- left_join(Phylo_species4, eco.species_wFactors, by = "scientific_name")


GLM_df$Euryhaline_Status <- fct_relevel(GLM_df$Euryhaline_Status, "Marine Only")



GLM_model <- glm(risk ~ TL_Log + Euryhaline_Status, data = GLM_df, family = binomial)

summary.glm(GLM_model)

hold <- summary.glm(GLM_model)$coefficients

hold2 <- data.frame(hold)

write.csv(hold2, file="GLM_Summary_Results.csv")
...
GLM <- read.csv("GLM_Summary_Results.csv")

GLM <- filter(GLM, !X == "(Intercept)")

#Calculate 95% CI
GLM$L95 <- GLM$Estimate - (GLM$Std..Error * 1.96)
GLM$U95 <- GLM$Estimate + (GLM$Std..Error * 1.96)


GLM_To_Merge <- GLM %>% 
  select(Category = X, Estimate, SE = Std..Error, L95, U95) %>% add_column(Test = "GLM")

PhyloGLM <- results2 %>% select(
  Category = Variable_Level, Estimate = Coef, SE, L95, U95) %>% add_column(Test = "PhyloGLM")

To_Graph <- rbind(GLM_To_Merge, PhyloGLM)


#Compare Figure----
#Figure 4
Compare <- ggplot(To_Graph, aes(x = Category, y = Estimate, fill = Test, color = Test)) +
  geom_point(alpha = .5, shape = 21, size = 5, position = position_dodge(width = .2)) +
  xlab("")+ylab("Extinction Risk (log-odds)")+
  scale_x_discrete(labels=c("Marine and Brackish","Marine, Brackish, and Freshwater","Total Length (log10)mm")) +
  expand_limits(y = c(-1.1, 2.1)) +
  scale_y_continuous(breaks=seq(-1,2,by=.5)) +
  geom_errorbar(aes(
    ymin = L95, ymax = U95), color = "Black", width = 0, position=position_dodge(width = .2)) +
  scale_fill_manual(values=c("Blue","Red"))+ theme_bw() +
  theme(legend.title=element_blank()) +
  theme(axis.text.x = element_text(angle = 16, vjust = 1, hjust=1)) +
  ...
  theme(axis.line = element_line(colour = "black"),
        ...
        panel.background = element_blank()) +                               # Change margins of ggplot2 plot
  theme(plot.margin = unit(c(t=1, r=0, b=0, l=3), "cm"))
...
# #Save image
png("Compare.png", width = 8, height = 8, units = "in", res = 600)
Compare
dev.off()



#Calculated Confidence Interval Widths in Excel, see Excel Doc titled
#"Best_Model_Results_Compare_CIW"




#Building table of N per model----
#Wanted to get an idea of how many observations we have for each model
#pulls out element of nested list, ie in this case the 3rd element in all nested lists
...
#N is 15
test<-lapply(model1,'[[',15)


N<- lapply(allModels_j, '[[',15)
formula <- lapply(allModels_j, '[[',17)

N_AllModels <- cbind(N, formula)

#write to csv
write.csv(N_AllModels, "N_by_Model.csv")
